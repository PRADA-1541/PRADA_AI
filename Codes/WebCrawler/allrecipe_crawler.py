...
if os.path.isfile(file_path):
    ...
    with open(file_path, 'w+') as line:
        pickle.dump(recipe_links, line)
...
for recipe in recipe_links:
    ...
    content = container.find_element(By.ID, 'loc.article-content')
    content_data = content.find_element(By.ID, 'article-content_1-0')
    recipe_details = content.find_element(By.ID, 'recipe_details_1-0')
    ...
    for item in recipe_details_items:
        recipe_details_label = item.find_element(By.CLASS_NAME, 'mntl-recipe-details__lable').text[:-1]
    ...
    ingredients_list = content_data.find_element(By.ID, 'mntl-structured-ingredients__list')
    ...
    #article_body = header.find_element()
